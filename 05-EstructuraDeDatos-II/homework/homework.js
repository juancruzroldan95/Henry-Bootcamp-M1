...
class LinkedList {
  ...
  remove() {
    ...
    if(this.len === 0)
      return null;
    if(this.len === 1) {
      let removedValue = pointerAux.value;
      this.head = null;
      this.len--;
      return removedValue;
    }
    while (pointerAux.next.next !== null) {
      pointerAux = pointerAux.next;
    }
    let removedValue = pointerAux.next.value;
    pointerAux.next = null;
    this.len--;
    return removedValue;
  }
...
La clase debe tener los siguientes métodos:
  ...
  cada caracter del input (investigar el método charCodeAt de los strings) y calcula el módulo de ese número total por la cantidad de
  buckets; de esta manera determina la posición de la tabla en la que se almacenará el dato.
